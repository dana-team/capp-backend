...
func TestBuildRelPath(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cappName:  "my-capp",
			want:      "sites/test1/my-namespace/my-capp.yaml",
		},
		{
			...
			namespace: "prod",
			cappName:  "web-app",
			want:      "sites/test2/prod/web-app.yaml",
		},
	}
...
func TestBuildRelPath_CustomPrefix(t *testing.T) {
	...
	got := c.BuildRelPath("test1", "ns", "app")
	assert.Equal(t, "overrides/test1/ns/app.yaml", got)
}
